...
6.2 充电逻辑
恒流充电闪红灯，电流15-20mA
高压进入涓流充电闪绿灯，电流2.5-10
截止电流2.5
充满灭灯

充满拔下继续充，会进入涓流充电，亮一段时间绿灯，然后再灭掉

6.3 戒指信息
尺寸6-13号

6.4 久坐提醒
当每天的固定时间段，子啊设定的分钟数内步数少于设定值时，戒指会通过手机提醒用户久坐

6.5 24小时心率测量间隔
每隔设定的分钟数，戒指会进行心率测量，数据会存储在戒指内，用户可通过APP想戒指请求数据并查看心率数据
指令格式：00 关闭 01 五分钟 06 30分钟

6.6 睡眠体温测量
如果戒指没有体温IC，每隔五分钟读取MCU温度
6.7 触控开关
通过触控IC触发蓝牙HID协议动作（双击、长按、上滑、下滑）
HID操作类型：00 短视频 01 音乐 02 拍照 03 阅读 04 幻灯片

6.8 摇一摇拍照
晃动戒指，戒指同步0317拍照指令给APP，APP执行拍照

6.9灰卡测试
灰卡是精确检测曝光量的基准
灰卡测试（Grey Card Test）是智能手环或其他可穿戴设备在产品设计与开发中用于检测心率监测传感器性能的一种方法。智能手环通常使用光电容积脉搏波描记法（PPG）来监测心率，该技术依赖于光线的反射或透射，利用绿色LED光和光电探测器检测血液流动。

在灰卡测试中，通常会使用不同灰度的卡片或表面来模拟不同的皮肤类型和颜色。这些卡片反射不同强度的光线，目的是验证心率传感器是否能够在不同的反射率条件下，依然保持准确、稳定的心率检测。灰卡测试有助于判断传感器在非人体条件下的性能稳定性，找出可能影响检测准确性的因素。

灰卡测试对设备厂商来说是一个重要的步骤，因为用户的皮肤颜色和穿戴环境可能会对心率监测的准确性造成影响，通过这种测试可以提高设备的适应性。
